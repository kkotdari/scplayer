 ...
 *    ?group=gameResult&item=<경기번호|id>&t=<초>&s=<배속>&z=&cx=&cy=&a=<각도>&tr=<추적 아이디>
 *  기본값(배속 1·배율 1·각도 90)은 안 싣는다 — 주소만 길어진다.
 *  보내기는 기기의 공유 시트(navigator.share)를 먼저 쓰고, 없거나 거절되면 링크를 복사한다. */
export default function SceneShareButton({ clockKey, item, title }: {
  clockKey: string; item: string; title: string;
}) {
  const [done, setDone] = useState<null | "shared" | "copied">(null);
  const buildUrl = (): string => {
    const q = new URLSearchParams();
    q.set("group", "gameResult");
    q.set("item", item);
